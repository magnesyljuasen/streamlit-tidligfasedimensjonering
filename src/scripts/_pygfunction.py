...
from src.scripts._utils import Plotting
...
class Simulation:
    ...
    def _properties(self):
        ...
        self.K_S = 3.0  # Ground thermal conductivity (W/m.K)            
        self.T_G = 8  # Undisturbed ground temperature (degrees)   
        self.K_G = 0.8  # Grout thermal conductivity (W/m.K)
        self.K_P = 0.42  # Pipe thermal conductivity (W/m.K)
        self.H = 300  # Borehole depth (m)
        self.B = 12  # Distance between boreholes (m)
        self.D = 10  # Borehole buried depth
        self.FLOW_RATE = 0.6  # Flow rate (kg/s)
